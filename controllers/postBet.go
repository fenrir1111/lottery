...
func PostBet(ctx iris.Context) {
	...
	//验证是否有这个彩票postBet.GameId在数据库
	if servicesLotto.LotteryIn(postBet.GameId) == false {
	...
	for i := 0; i < len(postBet.Bet_list); i++ {
		...
		fmt.Println(tempCount, "	", intBetCount, "	tmpF64BetCount=", tmpF64BetCount, " f64BetEachMoney=", f64BetEachMoney, "	", round(f64BetEachMoney*tmpF64BetCount), "	", f64BetMoney)
		if intBetCount != tempCount || round(f64BetEachMoney*tmpF64BetCount) != f64BetMoney {
			ctx.JSON(models.Result{Code: 202, Message: "下注错误!", Data: nil})
			return
		}
	...
	for more := 0; more < bets[0].BetMore; more++ { //追期
		//fmt.Println(bets[0].GamePeriod)
		switch bets[0].GameId {
		case 1: //重庆时时彩
